...
/**
 * Adding a customer class. Implements variables and methods for adding a customer to the database.
 *
 * @author James Watson - 001520415
 */
public class controllerCustomerAdd implements Initializable {
	/** gui buttons */
	...
	public void initialize(URL url, ResourceBundle resourceBundle) {
		...
		try {
			...
			while(customerSet.next()) {
				allId.add(customerSet.getString("Customer_ID"));
			}
	...
	/**
	 * Closes window without adding customer. Called when cancel button is clicked.
	 */
	public void buttonCancel() {
		controllerMain.message = "Customer was not added";
	...
	public void buttonSave() throws SQLException {
		if (inputValidated()) {
			...
			String creator = controllerMain.user;
			String divisionId = "";
			String divisionName = "";
			for (Location location : allLocation) {
			...
			Jdbc.updateDB("" +
				...
				"VALUES (\"" + id + "\", \"" + name + "\", \"" + address + "\", \"" + postal + "\", \"" + phone +
					"\", \"" + timeUTC + "\", \"" + creator + "\", \"" + timeUTC + "\", \"" + creator +
					"\", \"" + divisionId + "\")");
			controllerMain.allCustomers.add(new Customer(id, name, address, postal, phone, divisionName));
			controllerMain.customerIndex = (Integer.parseInt(id) - 1);
			controllerMain.message = "Customer added successfully";
